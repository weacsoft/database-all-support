...
import com.weacsoft.oracle.query.grammars.OracleGrammar;
...
public class OracleQueryBuilderConfig implements QueryBuilderConfig {
    @Override
    ...
    public <T, K> Builder<?, T, K> newBuilder(GaarasonDataSource gaarasonDataSource, Model<?, T, K> model) {
        return (Builder<?, T, K>) new OracleBuilder<>().initBuilder(gaarasonDataSource, ObjectUtils.typeCast(model), new OracleGrammar(model.getTableName()));
    }
}
